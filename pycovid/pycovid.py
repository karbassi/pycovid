...
def get_covid_cases(countries=None,
                    provinces=None,
                    start_date=None,
                    end_date=None,
                    casetype=None,
    ...
    if provinces is not None:
        for province in provinces:
            if province not in df.province_state.values:
                print(
                    f"Province: {province} not found in database. Check spelling!"
                )
    ...
    if countries is not None:
        for country in countries:
            if country not in df.country_region.values:
                print(
                    f"Country: {country} not found in database. Check spelling!"
                )
            df = df[(df.country_region.isin(countries))]

    if cumsum is True:
        df.cases = df.groupby("province_state")["cases"].transform(
            pd.Series.cumsum)
...
def get_covid_cases_wide(countries=None,
                         start_date=None,
                         end_date=None,
                         casetype=None,
                         cumsum=False):
    ...
    if cumsum is True:
        df.confirmed = df.groupby("country_region")["confirmed"].transform(
            pd.Series.cumsum)
        df.recovered = df.groupby("country_region")["recovered"].transform(
            pd.Series.cumsum)
        df.death = df.groupby("country_region")["death"].transform(
            pd.Series.cumsum)
...
def get_interval_data(df, interval="30D"):
    ...
    df = df.groupby([pd.Grouper(freq=interval),
...
def plot_provinces(countries=None,
                   provinces=None,
                   start_date=None,
                   end_date=None,
                   casetype=None):
    ...
    if provinces is not None:
        for province in provinces:
            ...
            df.loc[df.province_state == province, "cases"] = (
                df.loc[df.province_state == province,
                       "cases"] / province_populations[province]) * 100000
    ...
    fig = px.line(
        ...
        title="Number of confirmed COVID-19 cases over time per 100,000 Canadians")
